...
def cube_imp_no_batch(weights, indices_rs, pts, cnts, is_train, n_samples, **kwargs):
    pts_rs = cube_sample_pdf_no_batch(pts, cnts, weights[..., 1:-1], indices_rs, n_samples, is_train).detach()
    pts = torch.cat([pts, pts_rs + cnts[:, None]], 1)
    return {'pts' : pts, 'cnts' : cnts, 'dx' : kwargs['dx'], 'dy' : kwargs['dy'], 'dz' : kwargs['dz']}

def cube_sample_pdf_no_batch(pts, cnts, weights, indices_rs, N_samples, is_train):
    print(f"pts.shape={pts.shape}, cnts.shape={cnts.shape}, weights.shape={weights.shape}, indices_rs.shape={indices_rs.shape}")
    ...
    print(f"cfg.shape: {cdf.shape}, inds_g.shape: {inds_g.shape}, matched_shape: {matched_shape}")
