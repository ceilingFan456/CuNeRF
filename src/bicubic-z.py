...
def downsample_in_z(data, scale):
    # return data[::scale, ::scale, ::scale]

    H, W = data.shape[1], data.shape[2]
    xy_inds = torch.meshgrid(torch.linspace(0, H - 1, H), torch.linspace(0, W - 1, W))
    ...
    data = data.reshape(len(vals), H, W)
    return data
...
def save(gt, pd, save_path, scale):
    ...
    for i in range(n):
        ...
        print(f"frame:{i}, PSNR: {psnr_value:.2f}, SSIM: {ssim_value:.2f}")
        ...
        if i % scale == 0:
            ...
            psnrs_testing.append(psnr_value)
            ssims_testing.append(ssim_value)
    ...
    with open(os.path.join(save_path, 'logs.txt'), 'w') as f:
        ...
        f.write(f"PSNR Testing: {np.mean(psnrs_testing)}\n")
        f.write(f"SSIM Testing: {np.mean(ssims_testing)}\n")
        ...
        print(f"PSNR Testing: {np.mean(psnrs_testing)}")
        print(f"SSIM Testing: {np.mean(ssims_testing)}")
...
def main(args):
    ...
    down = downsample_in_z(data, args.scale)
